# Create a Genie Agent Space

Genie Agents let agents query structured data in Unity Catalog tables using natural language. A Genie Agent space can include up to 30 tables or views.

## Create via Databricks UI

1. In your workspace, go to **Genie** in the left sidebar.
2. Click **New** to create a new Genie Agent space.
...
The space ID is in the URL when viewing the Genie Agent space:

```
https://<workspace>.databricks.com/genie/rooms/<space-id>?o=...
```

To list all Genie Agents via CLI:
...
Wire the Genie Agent space into your agent and grant permissions. See the **add-tools** skill and use `examples/genie-space.yaml` for the `databricks.yml` resource grant.
