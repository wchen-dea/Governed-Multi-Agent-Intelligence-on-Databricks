...
if [ -n "${HITL_WAREHOUSE_ID:-}" ]; then
    WAREHOUSE_ID="$HITL_WAREHOUSE_ID"
elif [ "$HITL_ENV" = "dev" ]; then
    WAREHOUSE_ID="b20f70f71c2f52e2"
else
...
APP_JSON="$(databricks_cli apps get "$APP_NAME" --output json 2>/dev/null || true)"
...
grant_object() {
    ...
    object_full_name="$2"
    privilege="$3"
    run databricks_cli grants update "$object_type" "$object_full_name" \
        --json "{\"changes\":[{\"principal\":\"$SP_CLIENT_ID\",\"add\":[\"$privilege\"]}]}"
    printf "Granted %s on %s %s\n" "$privilege" "$object_type" "$object_full_name"
}

grant_warehouse() {
    run databricks_cli permissions update warehouses "$WAREHOUSE_ID" \
...
grant_orchestrator_can_use() {
    ...
    orchestrator_json="$(databricks_cli apps get "$ORCHESTRATOR_APP_NAME" --output json 2>/dev/null || true)"
    ...
    run databricks_cli apps update-permissions "$APP_NAME" \
