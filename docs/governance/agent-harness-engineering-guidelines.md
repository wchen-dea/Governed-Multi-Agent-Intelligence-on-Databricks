...
The harness is the `src/aiserver/api/`, `application/`, `bootstrap/`, and `infrastructure/` code that runs independently of any single model call: request lifecycle, dependency composition, deterministic policy/guardrail enforcement points, bounded agent-to-agent delegation, task execution, and audit/tracing plumbing.

## Implemented Mechanisms and Rules

### 1. Layered request pipeline

- Source: `src/aiserver/api/invocations.py` (`@invoke`/`@stream`), `src/aiserver/bootstrap/container.py` (DI wiring).
- Rule: request-time policy (`application/auth/policy.py`) runs before any tool call; response-time guardrails (`application/guardrails/checks.py`) run after the model produces output. Do not add ad hoc checks inside handlers — add them to the appropriate bounded context so both invoke and stream paths get the same enforcement.
- Rule: keep handler functions thin (parse/normalize input, call application use cases, shape output). Use concise concept filenames within a bounded-context folder, such as `agent.py`, `policy.py`, `checks.py`, or `tasks.py`.

### 2. Typed execution contracts

- Source: `src/aiserver/contracts/responses.py` (`RoutePlan`, `ToolExecutionResult`, `ResponseEnvelope`).
...
- Source: `src/aiserver/application/orchestration/routing.py` (`build_route_plan`, `MIN_ROUTE_CONFIDENCE`).
...
- Source: `src/aiserver/application/orchestration/model.py` (`ModelSelection`, `_REASONING_TERMS`, `_SYNTHESIS_TERMS`).
...
- Source: `src/aiserver/infrastructure/messaging/bus.py` (structured logging, noop, Kafka, RabbitMQ, UC table backends), `MESSAGE_BUS_*` settings.
...
- Source: `src/aiserver/application/auth/context.py`, subagent `auth_mode` field (`app` vs `obo`).
...
- Source: `mlflow.update_current_trace` through `src/aiserver/infrastructure/observability/tracing.py`.
...
- **Load/backpressure handling on the task bus.** `infrastructure/persistence/tasks.py` and `application/delegation/worker.py` bound a single worker per handoff; document and test behavior under many concurrent delegations (queueing, rejection, or backpressure signal) before this becomes a production incident.
- **Model routing evaluation.** `application/orchestration/model.py`'s term-set heuristics (`_REASONING_TERMS`, `_SYNTHESIS_TERMS`) aren't currently covered by the KPI release gate; add model-selection accuracy as an explicit eval dimension so heuristic changes are measured, not just unit-tested.
