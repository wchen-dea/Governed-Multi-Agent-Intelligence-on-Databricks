...
- **Secondary tracing hook:** `deepeval`'s `DeepEvalTracingProcessor` is also registered on the Agents SDK trace pipeline in [invocations.py](../../src/aiserver/api/invocations.py), additive to MLflow autologging (different instrumentation layer, no conflict). It's local-only today — no `CONFIDENT_API_KEY` is configured, so no trace data leaves the app; a key would opt into Confident AI's hosted dashboards/online evals.
