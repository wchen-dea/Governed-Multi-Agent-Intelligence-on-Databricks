...
class AgentTaskWorker:
    ...
    async def run_once(self) -> int:
        ...
        for record in claimed:
            task = record.task
            self._message_bus.publish("delegation.task.claimed", {"task_id": task.task_id, "correlation_id": task.correlation_id})
            ...
            if not decision.allowed:
                ...
                self._message_bus.publish("delegation.policy.rejected", {"task_id": task.task_id, "reason_code": decision.reason_code})
                continue
            await self._task_bus.mark_running(task.task_id, self._worker_id)
            try:
                output = await self._executor(task)
            except Exception as exc:
                record = await self._task_bus.fail(task.task_id, self._worker_id, type(exc).__name__)
                event_type = "delegation.task.dead_lettered" if record.status == "dead_letter" else "delegation.task.failed"
                self._message_bus.publish(event_type, {"task_id": task.task_id, "error_type": type(exc).__name__})
                continue
            result = DelegationResult(task_id=task.task_id, correlation_id=task.correlation_id, status="succeeded", output=output)
            await self._task_bus.complete(result, self._worker_id)
            self._message_bus.publish("delegation.task.completed", {"task_id": task.task_id, "correlation_id": task.correlation_id})
        return len(claimed)
